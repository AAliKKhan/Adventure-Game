    ...
    if (question2.direction === "North") {
      ...
      if (questionNorth1.action === "Run") {
        ...
        await inquirer.prompt([
          { type: "input", name: "Fight2", message: chalk.yellow("Shoot") },
        ]);
        console.log(chalk.yellow("Monster's health is getting low"));
        await inquirer.prompt([
          { type: "input", name: "Fight4", message: chalk.yellow("Shoot") },
        ]);
        console.log(chalk.yellow("Monster's health is getting low"));
      ...
      const questionSouth1 = await inquirer.prompt([
        {
          ...
          name: "action",
          message: chalk.red(
            "There is a trap on this path, step on the correct block to survive"
          ),
          choices: [1, 2, 3, 4],
        },
      ]);
      ...
      if (parseInt(questionSouth1.action) === randomNumber) {
        console.log(
          chalk.green(
            "Congratulations! You stepped on the correct block and survived."
          )
        );
      } else {
        console.log(
          chalk.red(
            "Oops! You stepped on the wrong block and fell into the trap, Game Over."
          )
        );
      }
    }
